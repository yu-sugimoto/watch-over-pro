...
struct PairingInputView: View {
    ...
    private func validateAndPair() async {
        ...
        guard let pairingCode = await pairingRepo.validate(fullCode) else {
            errorMessage = "無効なコードです。コードを確認してください。"
            isValidating = false
            return
        }
        ...
        if let name = personName,
           ...
           let age = personAge {
            await personRepo.updateInfo(
                pairingCode.personId,
                name: name,
                relationship: relationship,
                age: age,
                colorHex: personColorHex ?? "34C759",
                notes: personNotes ?? ""
            )
        }

        await pairingRepo.markUsed(pairingCode.id)

        let link = DeviceLink(
            personId: pairingCode.personId,
            watchedDeviceId: pairingCode.watcherDeviceId,
            watcherDeviceId: appModeManager.deviceId
        )
        await pairingRepo.createDeviceLink(link)

        if let name = personName {
            pairedPersonName = name
        } else {
            let fetched = await personRepo.fetchById(pairingCode.personId)
            pairedPersonName = fetched?.name ?? ""
        }
