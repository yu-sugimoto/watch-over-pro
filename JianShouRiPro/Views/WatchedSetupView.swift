...
struct WatchedSetupView: View {
    ...
    @State private var savedPersonId: UUID?
    ...
    private func generateCode() async {
        ...
        do {
            let personId: UUID
            if let existingId = appModeManager.linkedPersonId {
                personId = existingId
            } else if let saved = savedPersonId {
                personId = saved
            } else {
                ...
                let exists = await personRepo.exists(person.id)
                guard exists else {
                    errorMessage = "見守り対象の登録に失敗しました。ネットワーク接続を確認してください。"
                    isGenerating = false
                    return
                }
            ...
            let code = try await pairingRepo.generateCode(
                for: personId,
                deviceId: appModeManager.deviceId
            )
            appModeManager.linkToPerson(id: personId, name: "")
            generatedCode = code
        } catch {
