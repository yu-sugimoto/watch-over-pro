...
export async function handler(
  event: AppSyncResolverEvent<{ family_id: string }>,
) {
  const userId =
  ...
  const members = (membersResult.Items ?? []).map(
  ...
  const callerIsMember = members.some(
    (m: { member_user_id: string }) => m.member_user_id === userId,
  );
  if (!callerIsMember) {
  ...
  if (trackedUserIds.length > 0) {
    ...
    const keys = trackedUserIds.map((id: string) => ({
      tracked_user_id: { S: id },
    }));
    ...
    for (const item of locItems) {
      locations.push({
        ...
        battery_level: item.battery_level?.N
          ? Number(item.battery_level.N)
          : null,
